...
pub fn export_svg_badge(passed: usize, failed: usize, output_path: &str) -> Result<(), String> {
    ...
    // Entire SVG is a raw string. `{color}` and `{percent}` are filled by `format!`.
    let svg = format!(
        r#"<svg xmlns="http://www.w3.org/2000/svg" width="150" height="20" role="img" aria-label="validation">
